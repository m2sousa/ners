...
impl Cpu {
    pub(super) fn BRK(&mut self, _: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn LDA(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn TAX(&mut self, _: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn INX(&mut self, _: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn ORA(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn ASL(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let mut operand = self.get_instruction_operand(bus, mode);
        ...
        if matches!(mode, AddressingMode::Accumulator) {
            ...
            self.mem_write(bus, addr, operand);
        }
    ...
    pub(super) fn PHP(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.push_stack(bus, self.reg.status | StatusFlags::B);
    }

    /// If the negative flag is clear then add the relative displacement to the program counter to cause a branch to a new location.
    pub(super) fn BPL(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::NEGATIVE != 0 {
            return;
        }

        let relative_offset = self.get_instruction_operand(bus, mode) as i8;

        self.reg.pc = self.reg.pc.wrapping_add(relative_offset as u16);
    }

    /// Set the carry flag to zero.
    pub(super) fn CLC(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.unset(StatusFlags::CARRY);
    }

    /// The JSR instruction pushes the address (minus one) of the return point on to the stack and then sets the program counter to the target memory address.
    pub(super) fn JSR(&mut self, bus: &mut Bus, mode: AddressingMode) {
    ...
    pub(super) fn AND(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn BIT(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn ROL(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let mut operand = self.get_instruction_operand(bus, mode);
        let msb = operand & 0b1000_0000;
    ...
    pub(super) fn PLP(&mut self, bus: &mut Bus, _: AddressingMode) {
        // Note that PLP does ignore the B flag.
        self.reg.status = self.pull_stack(bus) & !StatusFlags::B;
    ...
    pub(super) fn BMI(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::NEGATIVE == 0 {
            return;
        }
    ...
    pub(super) fn SEC(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.set(StatusFlags::CARRY);
    }

    /// The RTI instruction is used at the end of an interrupt processing routine.
    /// It pulls the processor flags from the stack followed by the program counter.
    pub(super) fn RTI(&mut self, bus: &mut Bus, _: AddressingMode) {
        const RTI_LENGTH: u16 = 1;

        self.reg.status = self.pull_stack(bus) & !StatusFlags::B | StatusFlags::UNUSED;

        let lo = self.pull_stack(bus) as u16;
        let hi = self.pull_stack(bus) as u16;
    ...
    pub(super) fn EOR(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn LSR(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let mut operand = self.get_instruction_operand(bus, mode);
        let lsb = operand & 1;
    ...
    pub(super) fn PHA(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.push_stack(bus, self.reg.acc);
    }

    /// Sets the program counter to the address specified by the operand.
    pub(super) fn JMP(&mut self, bus: &mut Bus, mode: AddressingMode) {
        const JMP_LENGTH: u16 = 3;

        // Note that JMP do not must the same method as other instructions due to a cpu bug.
        let addr = self.get_jmp_operand_address(bus, mode);
    ...
    pub(super) fn BVC(&mut self, bus: &mut Bus, mode: AddressingMode) {
    ...
    pub(super) fn CLI(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.unset(StatusFlags::INTERRUPT_DISABLE);
    }

    /// The RTS instruction is used at the end of a subroutine to return to the calling routine.
    /// It pulls the program counter (minus one) from the stack.
    pub(super) fn RTS(&mut self, bus: &mut Bus, _: AddressingMode) {
        let lo = self.pull_stack(bus) as u16;
        let hi = self.pull_stack(bus) as u16;
    ...
    pub(super) fn ADC(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn ROR(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let mut operand = self.get_instruction_operand(bus, mode);
        let lsb = operand & 1;
    ...
    pub(super) fn PLA(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.acc = self.pull_stack(bus);

        self.set_zero(self.reg.acc);
        self.set_negative(self.reg.acc);
    }

    pub(super) fn BVS(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::OVERFLOW == 0 {
            return;
        }
    ...
    pub(super) fn SEI(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.set(StatusFlags::INTERRUPT_DISABLE);
    }

    /// Stores the contents of the accumulator register into memory.
    pub(super) fn STA(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let addr = self.get_operand_address(bus, mode);

        self.mem_write(bus, addr, self.reg.acc);
    }

    /// Stores the contents of the Y register into memory.
    pub(super) fn STY(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let addr = self.get_operand_address(bus, mode);

        self.mem_write(bus, addr, self.reg.y);
    }

    /// Stores the contents of the X register into memory.
    pub(super) fn STX(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let addr = self.get_operand_address(bus, mode);

        self.mem_write(bus, addr, self.reg.x);
    }

    /// Subtracts one from the Y register setting the zero and negative flags as appropriate.
    pub(super) fn DEY(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn TXA(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn BCC(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::CARRY != 0 {
            return;
        }
    ...
    pub(super) fn TYA(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn TXS(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.sp = self.reg.x;
    }

    /// Loads a byte of memory into the Y register setting the zero and negative flags as appropriate.
    pub(super) fn LDY(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn LDX(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn TAY(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn BCS(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::CARRY == 0 {
            return;
        }
    ...
    pub(super) fn CLV(&mut self, bus: &mut Bus, _: AddressingMode) {
        self.reg.unset(StatusFlags::OVERFLOW);
    }

    /// Copies the current contents of the stack register into the X register and sets the zero and negative flags as appropriate.
    pub(super) fn TSX(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn CPY(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn CMP(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn DEC(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let addr = self.get_operand_address(bus, mode);
    ...
    pub(super) fn INY(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn DEX(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn BNE(&mut self, bus: &mut Bus, mode: AddressingMode) {
    ...
    pub(super) fn CLD(&mut self, bus: &mut Bus, _: AddressingMode) {
    ...
    pub(super) fn CPX(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
    ...
    pub(super) fn SBC(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let operand = self.get_instruction_operand(bus, mode);
        let mut flag_carry = false;
    ...
    pub(super) fn INC(&mut self, bus: &mut Bus, mode: AddressingMode) {
        let addr = self.get_operand_address(bus, mode);
    ...
    pub(super) fn NOP(&mut self, bus: &mut Bus, _: AddressingMode) {}

    /// If the zero flag is set then add the relative displacement to the program counter to cause a branch to a new location.
    pub(super) fn BEQ(&mut self, bus: &mut Bus, mode: AddressingMode) {
        if self.reg.status & StatusFlags::ZERO == 0 {
            return;
        }
    ...
    pub(super) fn SED(&mut self, bus: &mut Bus, _: AddressingMode) {
